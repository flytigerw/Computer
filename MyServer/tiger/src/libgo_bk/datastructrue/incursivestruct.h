  ...
  template<class T>
    class WaitQueue{
      ...
      public:
        ...
        WaitQueue* check,*blocking_thresh_node;
        Functor check_fctor,convert_to_nonblocking;
        

    explicit WaitQueue(Functor check_functor = NULL,
            size_t thresh = -1, Functor const& convert = NULL)
        : blocking_thresh(thresh)
    {
        head = &dummy;
        tail = &dummy;
        check = &dummy;
        check_fctor = check_functor;
        blocking_thresh_node = nullptr;
        convert_to_nonblocking = convert;
        size = 0;
    }
    ...
    ConditionRet push(T* ptr,CondFunctor cond){
      std::unique_lock<LockType> lock_guard(lock);
      ConditionRet ret{true,true};
      //回调用户给的条件 ---> 条件针对队列中元素的数量
      if(cond){
        ret = cond(size);
        //不能排队
        if(!ret.can_queue)
      ...
      //当数量到达blocking_thresh时，后面的node都可在pop队首时变为非阻塞态，通过检查将其踢出等待队列中
      if(++size == blocking_thresh)
        blocking_thresh_node = ptr;

      if(size < (blocking_thresh+16))
        return ret;
      if(!check_fctor)
        return ret;

      if(!check || !check->next)
        check = blocking_thresh_node;

      //对blocking_thresh后面的元素，由于其可能处于非阻塞态，所以需要进行清理
      for(int i=0;check && check->next && i<2;++i){
        //检查点:check的next
        if(!check_fctor(static_cast<T*>(check->next))){
          if(blocking_thresh_node == check->next)
            blocking_thresh_node = blocking_thresh_node->next;
          if(tail == check->next)
            tail = check;
          auto temp = check->next;
          check->next = check->next->next;
          temp->next = nullptr;
          static_cast<T*>(temp)->decrement();
          continue;
        }
        check = check->next;
      }
    ...
    bool pop(T*& ptr){
      ...
      for(;;){
        if(!blocking_thresh_node || !convert_to_nonblocking)
          break;
        bool ok = convert_to_nonblocking(static_cast<T*>(blocking_thresh_node));
        blocking_thresh_node = blocking_thresh_node->next;
        if(ok)
          break;
      }
