...
namespace http {
  ...
  //从配置文件获取http的配置:request和response的buffer_size  ---> 限制协议文本大小
...
HttpRequestParser::HttpRequestParser()
    ...
    http_parser_init(&m_parser);          ///先对m_parser进行Init,然后再设置m_parser的一些回调----->解析时会自动调用 ---> 在此处即为，将解析的内容存放在HttpRequest中
...
size_t HttpRequestParser::execute(char* data, size_t len) {   //调用底层的execute,[0,offset]之间的内容为已解析内容 --> 使用mememove进行覆盖
...
void on_response_status(void *data, const char *at, size_t length) {
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    HttpStatus status = (HttpStatus)(atoi(at)); //atoi()
