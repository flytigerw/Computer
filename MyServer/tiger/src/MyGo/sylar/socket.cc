...
Socket::Socket(int family, int type, int protocol)
    ...
    ,m_type(type)
    ,m_protocol(protocol)
    ,m_isConnected(false) {  // ----> socket fd = -1 -----> 并没有立即创建 ---> fd为有限资源，在需要时通过init()手动创建
}
...
bool Socket::init(int sock) {
    ...
    SYLAR_LOG_INFO(g_logger) << "init(sock) get(sock) return nullptr";
    return false;
}

bool Socket::bind(const Address::ptr addr) {
  //若bind()时，当前socket无效，则创建一个socket fd
    if(!isValid()) {
        newSock();
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    //默认的timeout ---> 调用connect ----> connec使用默认的timout:5000
    ...
    } else {  //自定义timeout ---> 使用connect_with_timeout
...
bool Socket::close() {
    ...
    if(m_sock != -1) {
        ::close(m_sock);   
        m_sock = -1;            //底层的fd资源被释放，当前对象处于无效状态 ----> 但之后仍然可以通过init回归有效状态
    }
    return false;
}
...
Address::ptr Socket::getLocalAddress() {
    ...
    //先创建Address对象，再调用getsockname获取ip地址，再填充Address对象
    socklen_t addrlen = result->getAddrLen();
