  ...
  template<class Key,class HashFcn = hash<Key>,class EqualKey = equal_to<Key>,class Alloc = alloc>
    class hash_multiset{
      ...
      public :
        ...
        hash_multiset():hashtable(100,hasher(),key_equal()){}

        template<class InputIterator>
          hash_multiset(InputIterator first,InputIterator last,size_type n) : hashtable(n,hasher(),key_equal()){
            ht.insert_equal(first,last);
          }
        template<class InputIterator>
          hash_multiset(InputIterator first,InputIterator last) : hashtable(100,hasher(),key_equal()){
            ht.insert_equal(first,last);
          }
        //指定hash
        template<class InputIterator>
          hash_multiset(InputIterator first,InputIterator last,size_type n,const hasher& h) : hashtable(n,h,key_equal()){
            ht.insert_equal(first,last);
          }
