  ...
  template<class Key,class Value,class HashFcn = hash<Key>,class EqualKey = equal_to<Key>,class Alloc = alloc>
    class hash_map{
      ...
      public :
        ...
        hash_map():hashtable(100,hasher(),key_equal()){}

        template<class InputIterator>
          hash_map(InputIterator first,InputIterator last,size_type n) : hashtable(n,hasher(),key_equal()){
            ht.insert_unique(first,last);
          }
        template<class InputIterator>
          hash_map(InputIterator first,InputIterator last) : hashtable(100,hasher(),key_equal()){
            ht.insert_unique(first,last);
          }

        //指定hash
        template<class InputIterator>
          hash_map(InputIterator first,InputIterator last,size_type n,const hasher& h) : hashtable(n,h,key_equal()){
            ht.insert_unique(first,last);
          }
...
inline bool operator==(const hash_map<Value, HashFcn, EqualKey, Alloc>& hs1,
                       const hash_map<Value, HashFcn, EqualKey, Alloc>& hs2)
{
  return hs1.ht == hs2.ht;
}
template <class Val, class HashFcn, class EqualKey, class Alloc>
inline void swap(hash_map<Val, HashFcn, EqualKey, Alloc>& hs1,
                 hash_map<Val, HashFcn, EqualKey, Alloc>& hs2) {
  hs1.swap(hs2);
}
}
#endif
