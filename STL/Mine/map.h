...
//和map基本一致
//不同点在于:value_type为pair类型 --> 底层node存储着pair,红黑树的结构还是采用key进行构件
//将value_type(pair)传给KeyOfValue要能够得到pair.first --> key_type
...
namespace stl{
  ...
  template<class Key,class T,class Compare = less<Key>,class Alloc = alloc>
    ...
    //swap底层rb_tree的相关指向
    void swap(map<Key, Compare, Alloc>& x) { t.swap(x.t); }
